...
const OptionItem = styled.div<ThemeProps>`
  display: flex;
  align-items: center;
  ...
  box-shadow: ${({isDarkMode}) => isDarkMode 
    ...
    : '0 4px 12px rgba(255, 139, 167, 0.1)'};
  transition: all 0.2s ease;
  ...
  &:hover {
    ...
    box-shadow: ${({isDarkMode}) => isDarkMode 
      ...
      : '0 6px 16px rgba(255, 139, 167, 0.15)'};
  }
`;

const OptionText = styled.span<ThemeProps>`
  font-size: 1rem;
  color: ${({isDarkMode}) => isDarkMode ? '#E2E8F0' : '#FF8BA7'};
  font-weight: 500;
`;
...
const DeleteButton = styled.button<ThemeProps>`
  background: none;
  border: none;
  color: ${({isDarkMode}) => isDarkMode ? '#718096' : '#FFB3C6'};
  cursor: pointer;
  padding: 0.5rem;
  font-size: 1.2rem;
  transition: all 0.2s ease;
      ...
      <ContentContainer isDarkMode={isDarkMode}>
        ...
        <OptionList>
          {options.map((option, index) => (
            <OptionItem key={index} isDarkMode={isDarkMode}>
              <OptionText isDarkMode={isDarkMode}>{option}</OptionText>
              <DeleteButton onClick={() => handleDeleteOption(index)} isDarkMode={isDarkMode}>×</DeleteButton>
            </OptionItem>
          ))}
        </OptionList>
      </ContentContainer>
